...
struct MediumWidget_Quote_2Provider: TimelineProvider {
    
    func placeholder(in context: Context) -> MediumWidget_Quote_2Entry {
        print("Widget got loaded")
        return (MediumWidget_Quote_2Entry(date: Date(), quote: Quote(quote: "asdf", author: "adsf"), flowerImage: UIImage(named: "flower_10_babys breath_滿天星")!, flowerName: "asfd"))
    }
    
    
    func getSnapshot(in context: Context, completion: @escaping (MediumWidget_Quote_2Entry) -> Void) {
        
    
        Analytics.logEvent("widget_got_installed", parameters: nil)
        
        print("Widget got loaded")
        let quote = (MediumWidget_Quote_2Entry(date: Date(), quote: Quote(quote: "星星發亮是為了讓每一個人有一天都能找到屬於自己的星星", author: "小王子"), flowerImage: UIImage(named: "flower_10_babys breath_滿天星")!, flowerName: "滿天星"))
        completion(quote)
    }
    
    func getTimeline(in context: Context, completion: @escaping (Timeline<MediumWidget_Quote_2Entry>) -> Void) {
        ...
        if SyncAppQuotes().checkIfUpdate_widget()
        {
         print("update widget")
            ...
            DispatchQueue.main.async {
                let entry = MediumWidget_Quote_2Entry(date: Date(), quote: Quote(quote: "點開查看今日給你的話吧", author: "點開查看"), flowerImage: UIImage(named: "noun_seeds_184642")!, flowerName: "我是種子")
        ...
        }else
        {
            ...
            let entry = MediumWidget_Quote_2Entry(date: Date(), quote: Quote(quote: Q, author: A), flowerImage: FlowerImage, flowerName: FlowerName)
            let timeline = Timeline(entries: [entry], policy: .after(refreshDate))
            completion(timeline)
        }
...
struct MediumWidget_Quote_2Entry: TimelineEntry {
    var date: Date
    let quote: Quote
...
struct MediumWidget_Quote_2EntryView : View {
...
struct MediumWidget_Quote_2: Widget {
    private let kind = "MediumWidget_Quote_2"
    ...
    public var body: some WidgetConfiguration {
        StaticConfiguration(
            kind: kind,
            provider: MediumWidget_Quote_2Provider()
        ) { entry in
            MediumWidget_Quote_2EntryView(date: entry.date ,quote: entry.quote, flowerImage: entry.flowerImage, flowerName: entry.flowerName, quoteSize: 20, authorSize: 14)       .frame(maxWidth: .infinity, maxHeight: .infinity)    // << here !!
...
struct MediumWidget_Quote_2_Previews: PreviewProvider {
